...
<div class="flex items-center justify-center min-h-screen bg-gray-900">
    ...
    <form action="{{route('users.index')}}" method="get">
        ...
        <p class="text-red-500 text-xs mt-1">@if($errors->has('email')){{$errors->first('email')}}</p>@endif
    
        <label for="password" class="label">Password:</label>
        <input type="password" name="password" id="password" class="input">
        <p class="text-red-500 text-xs mt-1">@if($errors->has('password')){{$errors->first('password')}}</p>@endif
        ...
        <div class="flex items-center justify-between mb-4">
            ...
            <div class="flex items-center text-sm">
                ...
                <a href="{{route('users.create')}}" class="underline cursor-pointer ml-1">Register</a>
            </div>
        </div>
        @if (Session::has('login_error'))
            <p class="text-red-500 text-xs mt-1">{{Session::get('login_error')}}</p>
        @endif
